...
import { Folder, FileText, ChevronRight, ChevronDown, Search, X, Copy, Check, Play, Terminal, Info, Loader2, ExternalLink, Download, RefreshCw, Sparkles, Cpu, BarChart2, Sliders, GitFork, Server, ExternalLink as LaunchIcon, Globe } from 'lucide-react';
...
export function RepoExplorer({ repoName, rootPath, files }: RepoExplorerProps) {
  ...
  // View Modes & Panels
  const [viewMode, setViewMode] = useState<'code' | 'flowchart' | 'gee'>('code');
                  ...
                  {selectedFile.endsWith('.py') && (
                    <>
                      ...
                      <button 
                        ...
                        className="flex items-center gap-1.5 px-2.5 py-1 bg-emerald-600/20 text-emerald-400 border border-emerald-500/30 rounded-md text-[10px] font-bold hover:bg-emerald-600/30 transition-all disabled:opacity-50"
                      >
                        {isConvertingGee ? <Loader2 size={12} className="animate-spin" /> : <Globe size={12} />}
                        Export to GEE JS
                      </button>
                      <button 
                        onClick={() => explainCode()}
                            ...
                            <div className="flex items-center gap-2.5">
                              ...
                              <div>
                                <h4 className="text-xs font-bold text-slate-100 flex items-center gap-1.5">
                                  <span>Google Earth Engine JavaScript Code Editor</span>
                                ...
                                <p className="text-[10px] text-slate-400">
                                  Ready to paste directly into <a href="https://code.earthengine.google.com/" target="_blank" rel="noreferrer" className="text-emerald-400 hover:underline font-semibold">code.earthengine.google.com</a>
                                </p>
                              </div>
                            </div>
